...
int main(int argc, char** argv)
{
  ...
  if (result != libremidi::reader::invalid)
  {
	...
	std::cout << "Tick: " << tickDuration << " usecs, "
			  ...
			  << "Duration: " << duration / 60 << ":" << std::setw(2) << std::setfill('0') << duration % 60;
        ...
        if(event.m.is_meta_event())
        {
          ...
          switch(event.m.get_meta_event_type())
          {
            case libremidi::meta_event_type::SEQUENCE_NUMBER:
            	std::cout << "SEQUENCE_NUMBER " << (((uint16_t) event.m.bytes[3]) << 8) + event.m.bytes[4];
            	break;
            ...
            case libremidi::meta_event_type::TEMPO_CHANGE:
            	std::cout << "TEMPO_CHANGE: " << std::dec << ((((uint32_t) event.m.bytes[3]) << 16) + (event.m.bytes[4] << 8) + event.m.bytes[5]) << " usec / beat, ";
            	beatDuration = ((((uint32_t) event.m.bytes[3]) << 16) + (event.m.bytes[4] << 8) + event.m.bytes[5]); // [usecs]
            ...
            case libremidi::meta_event_type::TIME_SIGNATURE:
            	std::cout << "TIME_SIGNATURE: " << (int) event.m.bytes[3] << "/" << (int) pow(2., event.m.bytes[4])
						  << ", " << (int) event.m.bytes[5] << " clocks per beat, " << (int) event.m.bytes[6] << " 32nd notes per quarter note";
            	break;
