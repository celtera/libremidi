...
try
{
  ...
  // First we set ourselves in manual mode
  midiin.set_processing_mode(rtmidi::MANUAL);
  ...
  while(1)
  {
    // Here polling is done in this thread in blocking mode.
    // The first time a midi message is received we'll quit the program.
    if(!midiin.poll(10ms))
      return 1;
  }
}
