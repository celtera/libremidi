...
struct jack_helpers
{
  static bool check_port_name_length(
      const error_handler& self, std::string_view clientName, std::string_view portName)
  {
  ...
  static std::string
  get_port_name(const error_handler& self, const char** ports, unsigned int portNumber)
  {
    // Check port validity
    if (ports == nullptr)
    {
