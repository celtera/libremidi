...
inline constexpr port_handle raw_to_port_handle(port_id id) noexcept
{
  return (uint64_t(id.card) << 32) + (uint64_t(id.dev) << 16) + id.port;
}
inline constexpr port_id raw_from_port_handle(port_handle p) noexcept
{
  port_id ret;
...
struct helpers
{
  ...
  struct snd_ctl_wrapper
  {
    ...
    snd_ctl_wrapper(enumerator& self, const char* name)
    {
      ...
      {
        self.error(
            "helpers::enumerator::snd_ctl_wrapper: "
  ...
  struct enumerator
  {
    ...
    // < 0: error
    int is(snd_ump_stream_t stream, snd_ctl_t* ctl, int card, int device, int sub)
    {
      snd_ump_info_t* info;

      snd_ump_info_alloca(&info);
      snd_ump_info_set_device(info, device);
      snd_ump_info_set_subdevice(info, sub);
      snd_ump_info_set_stream(info, stream);
      ...
      {
        error(
            "helpers::enumerator::is: cannot get rawmidi information:", card, device, sub,
            snd_strerror(status));
        return status;
      }
      else
    ...
    {
      return is(SND_UMP_STREAM_INPUT, ctl, card, device, sub);
    }

    int is_output(snd_ctl_t* ctl, int card, int device, int sub)
    {
      return is(SND_UMP_STREAM_OUTPUT, ctl, card, device, sub);
    }
    ...
    void enumerate_cards()
    {
      ...
      {
        error(
            "helpers::enumerator::enumerate_cards: "
    ...
    void enumerate_devices(int card)
    {
      ...
      do
      {
        ...
        {
          error(
              "helpers::enumerator::enumerate_devices: "
    ...
    void enumerate_subdevices(snd_ctl_t* ctl, int card, int device)
    {
      ...
      snd_ump_info_set_stream(info, SND_UMP_STREAM_INPUT);
      snd_ctl_rawmidi_info(ctl, info);
      const int subs_in = snd_ump_info_get_subdevices_count(info);

      snd_ump_info_set_stream(info, SND_UMP_STREAM_OUTPUT);
      snd_ctl_rawmidi_info(ctl, info);
      const int subs_out = snd_ump_info_get_subdevices_count(info);

      alsa_raw_port_info d;
      d.card = card;
      d.dev = device;
      d.card_name = get_card_name(card);
      d.device_name = snd_ump_info_get_name(info);
      ...
      auto read_subdevice_info = [&](int sub) {
        ...
        d.subdevice_name = snd_ump_info_get_subdevice_name(info);
        d.sub = sub;
      };
